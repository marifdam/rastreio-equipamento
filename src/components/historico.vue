<template>
  ...
  const headers = [
    ...
    { title: "Status", align: "start" as const, value: "status", sortable: false },
  ];
  const input = ref()
  ...
  async function search(model: string, input: string) {
    const data = await store.dispatch("search", { selection: model, input: input });
  ...
  onMounted(async () => {
    ...
    if (input.value) {
      const data = await store.dispatch("search", { selection: "model", input: input.value })
      items.value = data.state;
    }
  });
